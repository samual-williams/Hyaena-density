...
### D closed
# null
mClosedD1 <- secr.fit(cptr_hst, model=list(D~1, g0~session), mask=maskClosed, detectfn=1, CL=TRUE)

# is density affected by site
mClosedD2 <- secr.fit(cptr_hst, model=list(D~session, g0~session), mask=maskClosed, detectfn=1, CL=TRUE) 
...
mClosedD6 <- secr.fit(cptr_hst, model=list(D~ReserveSize, g0~session), mask=maskClosed, detectfn=1, CL=TRUE) 

# is density affected by prey RAI
mClosedD7 <- secr.fit(cptr_hst, model=list(D~PreyRAI, g0~session), mask=maskClosed, detectfn=1, CL=TRUE) 

# is density affected by fence permeability
# mClosedD8 <- secr.fit(cptr_hst, model=list(D~Impenetrable, g0~session), mask=maskClosed, detectfn=1, CL=TRUE) # Would have same error as g0 model with this covariate - omit
...
# is density affected by prey RAI
mOpeng08 <- secr.fit(cptr_hst, model=g0~PreyRAI, mask=maskOpen, detectfn=1, CL=TRUE) 

# # is magnitude of detection affected by fence permeability
# mOpeng09 <- secr.fit(cptr_hst, model=g0~Impenetrable, mask=maskOpen, detectfn=1, CL=TRUE) 
...
# is density affected by leopards
mOpenD3 <- secr.fit(cptr_hst, model=list(D~LeopardRAI, g0~session), mask=maskOpen, detectfn=1, CL=TRUE) 
...
# # is density affected by fence permeability
# mOpenD8 <- secr.fit(cptr_hst, model=list(D~Impenetrable, g0~session), mask=maskOpen, detectfn=1, CL=TRUE) # Would have same error as g0 model with this covariate - omit
